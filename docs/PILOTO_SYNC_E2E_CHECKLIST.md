...
- [ ] Conectar red; esperar sincronización automática o pulsar «Reintentar sincronización» en Historial (si hay error).
- [ ] Historial: estado `synced`.
- [ ] Panel web: aparece la cotización con mismo código y `mobileId` coherente.

## Caso C — Doble sincronización (idempotencia)

- [ ] Con una cotización `pending`, forzar dos intentos de sincronización seguidos (p. ej. reintentar en Historial).
- [ ] En web solo existe **un** registro para ese `mobileId`.
- [ ] En móvil queda `synced` sin error.

## Caso D — Dispositivo revocado

- [ ] Revocar dispositivo en administración web.
- [ ] En móvil, intentar sincronizar cotización pendiente.
...
- [ ] Sincronización omitida: mensaje en Historial indicando usuario no vinculado al sistema central.

## Caso F — Recuperación `syncing` atascado

- [ ] Simular interrupción: forzar `sync_status = 'syncing'` en SQLite (o cerrar app durante sincronización).
- [ ] Iniciar sincronización de nuevo.
- [ ] La fila vuelve a `pending` y se reintenta; termina en `synced` o `error`, nunca permanece en `syncing`.

## Caso G — Diagnóstico en app

- [ ] Dashboard: banners unificados (config + cola de sincronización, máx. 2).
